...
func _input(event):
	if event.is_action_pressed("interact") && in_range == true:
		speak(quest_progress, lines_spoken)
...
func _on_Tallia_body_entered(body):
	if body.name == "Player":
		...
		tween.tween_property($Popup, "scale", Vector2(1, 1), 0.5).from_current().set_ease(Tween.EASE_IN_OUT)

func _on_Tallia_body_exited(body):
	if body.name == "Player":
		in_range = false
		var tween = get_node(".").create_tween() #create tween for popup's scale from 1->0 and ease in/out
		tween.tween_property($Popup, "scale", Vector2.ZERO, 0.5).from_current().set_ease(Tween.EASE_IN_OUT)
